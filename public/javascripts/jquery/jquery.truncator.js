...
(function($) {
  ...
  $.fn.truncate = function(options) {
    ...
    $(this).each(function() {
      ...
      var actual_max_length = opts.max_length - opts.more.length - 1;  // 1 for " " before 
      ...
      truncated_node.find('p:last').add(truncated_node).eq(0).
        append(' <a href="#show more content" title="Click for more">'+opts.more+'</a>');

      full_node.hide();
      full_node.find('p:last').add(full_node).eq(0).
        append(' <a href="#show less content" title="Click for less">'+opts.less+'</a>');
