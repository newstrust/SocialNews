class <%= model_controller_class_name %>Controller < ApplicationController
  ...
  def create
    ...
    if @<%= file_name %>.errors.empty?
      ...
      redirect_back_or_default('/start')
...
<% if options[:include_activation] %>
  def activate
    ...
    if logged_in? && !current_<%= file_name %>.active?
      ...
      flash[:notice] = "Sign up complete!<br>Please fill in your profile, to increase your member level."
    end
    redirect_back_or_default('/members/my_account#profile')
  end
