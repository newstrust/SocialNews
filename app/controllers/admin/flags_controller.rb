class Admin::FlagsController < ApplicationController
  ...
  all_access_to :except => [:index]
  grant_access_to :admin, :only => [:index]
  ...
  def create
    ...
    respond_to do |format|# note: you'll need to ensure that this route exists
      format.html { redirect_to ['admin',flag.flaggable] }
      format.json do
  ...
  def index
    @reason = params[:sort_by]  || 'flag'
    @flags = Flag.paginate(:all, pagination_params(:order => "id desc", :conditions => ["reason = ?", @reason], :include => :flaggable))
